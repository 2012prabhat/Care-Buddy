...
function Availability() {
    ...
    const deleteSlots = async (time, date, isBooked) => {
      if (isBooked) return alertError("This slot is booked you need to cancel your appointment first");
      
      const confirmDeletion = await alertConfirm(`Are you sure to delete slot of ${time} on ${new Date(date).toLocaleDateString()} ?`)
    ...
    const handleAddSlots = async (e)=>{
        ...
        let isTimeCorrect = isEndTimeAfterStartTime(formData.startTime, formData.endTime);
        console.log(isTimeCorrect)
        if(!isTimeCorrect){
          return alertError('Start time must be before end time');
        }
        
        const slots = generateTimeSlots(formData.startTime, formData.endTime, parseInt(formData.examineTime));
        const confirmation = await alertConfirm(`Are you sure you want to add slots from ${formData.startTime} to ${formData.endTime} on ${formData.date}`)
        if(confirmation.isConfirmed){
          try{
              const resp = await api.post('/doctors/add-slots',{date:formData.date,slots});
    ...
    function generateTimeSlots(startTime, endTime, examineTime) {
      ...
      for (let current = startMinutes; current <= endMinutes; current += examineTime) {
  ...
  return (
    ...
    <div className='sticky top-0  flex justify-between items-center'>
      ...
      <button className="flex items-center px-6 py-1 h-8 bg-[var(--iconCol)] text-white font-semibold rounded shadow hover:bg-[var(--iconColHover)] transition duration-200" onClick={()=>setAddAvailDis(true)}>
      <FontAwesomeIcon icon={faPlus} className="mr-2" />
      Add
    </button>
    </div>
    <div className='pb-16'>{availableSlots?.map((m,i)=>{
      return m.slots.length>0 &&
      <div key={i} className='mt-4'>
        <div className='border-b-2 border-blue-500 rounded p-2 w-fit'>Date - {new Date(m.date).toLocaleString("en-US",{ year: "numeric",
  month: "long",
  day: "numeric"})}</div>
        <div className='flex gap-x-10 flex-wrap'>{m.slots.map((slot,idx)=>{
          return <div key={idx} className={`mt-4 bg-white p-2 min-w-28 flex justify-between items-center rounded-md ${slot.isBooked && 'text-green-500'} `}>{slot.time} <FontAwesomeIcon onClick={()=>deleteSlots(slot.time,m.date,slot.isBooked)} className='cursor-pointer ml-2' color='indianred' icon="fa-solid fa-xmark" /></div>
        })}</div>
      </div>
    })}</div>
    ...
    <Modal isOpen={addAvailDis} onClose={()=>setAddAvailDis(false)} title="Add Availability" content={
      <>
